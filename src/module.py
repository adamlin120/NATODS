...
class NATODS(LightningModule):
    ...
    def forward_attentions(self, decoders, token_embed,
        ...
        for _ in range(self.hparams.num_passes):
            z_ds, _ = decoders[0].forward(
                query=token_embed, key=token_embed, value=token_embed,
            )
            z_ds, _ = decoders[1].forward(
                query=z_ds, key=delex_history_embed, value=delex_history_embed,
            )
            z_ds, _ = decoders[2].forward(
                query=z_ds, key=history_embed, value=history_embed,
            )
        return z_ds
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for value_logit, gt_encoded_value, fertility, domain, slot in zip(
            ...
            pred_values: List[str] = self.tokenizer.decode(
                value_logit.argmax(-1).tolist()).split()[:fertility]
